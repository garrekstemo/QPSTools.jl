...
テンプレート（`templates/plmap_analysis.jl`）は2パスのフローです：

**1回目** — 生スペクトルを見てPLピークを探す：
```julia
PIXEL_RANGE = nothing  # 1回目はnothingのまま
```
`spectra.pdf`が保存されます。数箇所の位置でCCDスペクトル全体が表示されるので、
PL発光を特定します（通常、最も広いピーク）。
そのピークを囲むピクセル範囲をメモします。

**2回目** — ピクセル範囲を設定してマップを作成：
```julia
PIXEL_RANGE = (950, 1100)  # PLピークを含むピクセル範囲
```
スクリプトは以下を実行します：
1. **バックグラウンド差し引き** — フレーク外（PLなし）の角のスペクトルを平均して
   全グリッド点から差し引き、レーザー散乱とノイズを除去
2. **ピクセル窓の積分** — 各グリッド点で`PIXEL_RANGE`内のカウントを合計し、
   PL強度を1つの値として取得
3. **正規化** — [0, 1]にスケーリング

ピクセル窓の外にあるもの（レーザー、ラマンピーク）は無視されます。
