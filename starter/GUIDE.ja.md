...
3. **バックグラウンド差し引き** — CCDスペクトルにはレーザー散乱、検出器の
   暗電流、迷光によるベースラインが含まれます。`subtract_background`は
   PLシグナルのない領域のスペクトルを平均してベースラインを推定し、
   全グリッド点から差し引きます。差し引き後はPL成分のみが残ります。

   2つのモード：
   - **自動**（デフォルト）：グリッドの下端の角のスペクトルを平均。
     スキャンの端がフレーク外（基板のみ）であることを前提とします。
     ```julia
     m = subtract_background(m)
     ```
   - **手動**：フレーク外の位置を空間座標で指定。
     フレークがグリッドの端まで広がっている場合に使用。
     ```julia
     m = subtract_background(m; positions=[(-40, -40), (40, -40)])
     ```

   解析テンプレートは確認用プロット（`spectra_corrected.png`）を保存するので、
   PLピークがフラットなベースライン上にあることを確認できます。
